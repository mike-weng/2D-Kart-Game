...
/**
 * The status panel. Renders itself to a fixed position in the bottom-right
 * corner of the screen, displaying the player's race ranking and item. (See the
 * render() method.)
 */
public class Panel {
	...
	/**
	 * Turn a cardinal number into an ordinal. For example, takes a ranking such
	 * as 2, and returns a String "2nd". Note: Won't work for numbers greater
	 * than 20.
	 */
	 ...
	 * @param ranking
	 *            The player's current position in the race (1 = 1st, 2 = 2nd,
	 *            etc).
	 * @param item
	 *            The player's currently-held item, or null.
	 */
	public void render(Graphics g, int ranking, Item item) {
		...
		if (item != null)
			item.drawImage(panel_left + 32, panel_top);
	}
}
